...
iclabelmenu = uimenu( plotmenu, 'label', 'ICLabel','userdata', 'startup:off;study:on');
...
uimenu( iclabelmenu, 'label', 'Label components', ...
    'callback', [try_strings.no_check ...
        ...
        'try, pop_viewprops(EEG, 0); catch,' ...
        'try, vp_path = fullfile(fileparts(which(''iclabel'')), ''viewprops'');' ...
        'addpath(vp_path); if length(EEG) == 1, LASTCOM = pop_viewprops(EEG, 0); end; rmpath(vp_path);' ...
        'disp(''ICLabel: Install the viewprops eeglab plugin to create IC visualizations like these elsewhere.''),' ...
        catch_strings.add_to_hist ' end'], 'userdata', 'startup:off;study:on');

uimenu( iclabelmenu, 'label', 'Flag components as artifacts', ...
    'callback', [try_strings.no_check '[EEG, LASTCOM] = pop_icflag(EEG);' catch_strings.store_and_hist ], 'userdata', 'startup:off;study:on');
